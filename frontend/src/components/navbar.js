...
const Navbar = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(true); // Status login pengguna
  ...
  useEffect(() => {
    if (isActive(pathname)) {
      setIsDropdownVisible(false); // Menyembunyikan dropdown ketika halaman aktif
    }
  }, [pathname]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dropdown Menu */}
        <div
          className={`bg-gray-800 bg-opacity-90 p-4 rounded-lg shadow-lg mt-1 absolute -translate-y-1/24 -translate-x-6 left-0 top-full z-20 w-48 transform transition-all duration-200 ease-in-out ${
